<nav class="bg-[#169E63] border-grey-200 px-2 sm:px-4 py-2.5 rounded">
    <div class="container flex flex-wrap justify-between items-center mx-auto">
        <a href="{{ url('/') }}" class="flex items-center">
            <span class="self-center text-[#ffffff] text-3xl font-extrabold whitespace-nowrap">KU จะแจ้ง!</span>
        </a>
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul class="flex flex-col p-4 mt-4 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white ">
                @auth
                    <li>
                        <p class="block py-2 pr-4 pl-3 rounded md:p-0 font-bold">
                            {{ Auth::user()->name }}
                        </p>
                    </li>
                    <li>
                    ...
                    <li>
                        <a href="{{ route('stats.index') }}"
                           class="block py-2 pr-4 pl-3 rounded md:p-0 hover:underline @if(Route::currentRouteName() === 'stats.index') current-page @endif" >
                            สถิติ
                        </a>
                    </li>
                    @endcan
                    ...
                    <li>
                        <a href="{{ route('appeals.index') }}"
                           class="block py-2 pr-4 pl-3 rounded md:p-0 hover:underline @if(Route::currentRouteName() === 'appeals.index') current-page @endif" >
                            สถานะการร้องเรียน
                        </a>
                    </li>
                    @endcan
